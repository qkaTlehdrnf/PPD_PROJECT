...
def factory_best_cases(factory, batch_size, training_size):
    ...
    for i in (pbar := trange(batch_size)):
        ...
        for batch_idx in range(training_size):
            ...
            if not batch_idx % (training_size//100):
                pbar.set_description('{0:>2}%'.format(int(batch_idx/training_size*100)+1))
            if best_loss > cost:
...
def factory_bestcases2better(factory, best_cases, steps, batch_size):
    ...
    for k in (pbar := trange(len(best_cases))):
        batch = best_cases[k]
        def_loss, batch = factory_batch_change2min(factory, batch, steps= steps, batch_size = batch_size)
...
if __name__ == "__main__":
    ...
    steps = [3,2,1]
    best_model = None
    best_cases = factory_best_cases(ICN, batch_size=batch_size, training_size=training_size)
    best_case = factory_bestcases2better(factory= ICN, best_cases= best_cases, steps= steps, batch_size= batch_size)
    print(best_case)
